...
class ChatController extends GetxController {
  ...
  final firestore.FirebaseFirestore firestoreInstance = firestore.FirebaseFirestore.instance;
  ...
  Future<void> addMessage(ChatMessage message) async {
    await firestoreInstance.collection('chat_messages').doc(message.id).set(message.toJson());
  ...
  Future<String> getDialogflowResponse(String text) async {
    ...
    final client = await auth_io.clientViaServiceAccount(accountCredentials, scopes);
    final projectId = 'chatbot-kyan';
    var uuid = Uuid();
    String sessionId = uuid.v4();
    final apiEndpoint = 'https://dialogflow.googleapis.com/v2/projects/$projectId/agent/sessions/$sessionId:detectIntent';
    ...
    if (response.statusCode == 200) {
      ...
      print('Failed to get response from Dialogflow. Status code: ${response.statusCode}');
